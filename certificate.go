...
func getDefaultCert(cancelCtx context.Context) ([]byte, error) {
	if len(cetkData) >= 0x350+0x300 {
		return cetkData[0x350 : 0x350+0x300], nil
	}
	cetkDir := path.Join(os.TempDir(), "cetk")
	if err := downloadFile(cancelCtx, nil, false, "http://ccs.cdn.c.shop.nintendowifi.net/ccs/download/000500101000400a/cetk", cetkDir, true); err != nil {
		return nil, err
	}
...
func GenerateCert(tmdData []byte, contentCount uint16, cancelCtx context.Context) (bytes.Buffer, error) {
	cert := bytes.Buffer{}
	...
	defaultCert, err := getDefaultCert(cancelCtx)
	if err != nil {
		return bytes.Buffer{}, err
	}
